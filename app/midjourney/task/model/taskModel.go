...
var (
	...
	taskImagineRowsWithPlaceHolder = strings.Join(stringx.Remove(taskFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`"), "=?,") + "=?"
)
...
func (m *defaultTaskModel) InsertByImagine(ctx context.Context, data *Task) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.TaskId, data.ParentTaskId, data.Action, data.Index, data.Prompt, data.ImageUrl, data.Description, data.Status, data.State, data.CreatedAt, data.UpdatedAt)
